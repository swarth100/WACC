...
func (rhs PairElemRHS) ASTString(indent string) string {
	if rhs.snd {
		return addIndentForFirst(indent, "SND",
			rhs.expr.ASTString(getGreaterIndent(indent)))
	} else {
		return addIndentForFirst(indent, "FST",
			rhs.expr.ASTString(getGreaterIndent(indent)))
	}
}
...
func (stmt AssignStatement) ASTString(indent string) string {
	...
	var innerIndent string = getGreaterIndent(indent)
	var lhsIndent string = addIndentForFirst(innerIndent, "LHS",
		stmt.target.ASTString(getGreaterIndent(innerIndent)))
	var rhsIndent string = addIndentForFirst(innerIndent, "RHS",
...
func (stmt IfStatement) ASTString(indent string) string {
	...
	var ifStats string = fmt.Sprintf("%vIF\n", addMinToIndent(indent))
	var condStats string = fmt.Sprintf("%vCONDITION\n",
		addMinToIndent(innerIndent))
	var thenStats string = fmt.Sprintf("%vTHEN\n", addMinToIndent(innerIndent))
	var elseStats string = fmt.Sprintf("%vELSE\n", addMinToIndent(innerIndent))

	var stmtStats string = stmt.cond.ASTString(doubleInnerIndent)
	...
	return fmt.Sprintf("%v%v%v%v%v%v%v",
		ifStats, condStats, stmtStats, thenStats, trueStats, elseStats, falseStats)
}
...
func (stmt WhileStatement) ASTString(indent string) string {
	...
	var condStats string = addIndentForFirst(innerIndent, "CONDITION",
...
func (fd FunctionDef) ASTString(indent string) string {
	...
	declaration := addIndAndNewLine(indent, fmt.Sprintf("%v %v(%v)",
		fd.returnType, fd.ident, params))
...
func (elem ArrayElem) ASTString(indent string) string {
	...
	for _, index := range elem.indexes {
		tmpIndex = addIndentForFirst(nextIndent, "[]",
...
func addArrayIndent(indent string, a1 string, arr []string) string {
	...
	for _, element := range arr {
		innerStats = fmt.Sprintf("%v%v", innerStats,
...
func addTripleIndentOnlyFst(ind string, a string, b string, c string) string {
	var innerStats2 string = c

	var typeStats string = addIndAndNewLine(ind, a)
	var innerStats string = b
	if c != "" {
		innerStats2 = c
	}
