...
func (stmt IfStatement) ASTString(indent string) string {
	...
	var innerIndent string = fmt.Sprintf("%v%v", indent, basicIdent)
	var doubleInnerIndent string = fmt.Sprintf("%v%v", innerIndent, basicIdent)
...
func (fd FunctionDef) ASTString(indent string) string {
	...
	innerIndent := fmt.Sprintf("%v%v", indent, basicIdent)
...
func addAtGreaterIndent(indent string, value string) string {
	return fmt.Sprintf("%v%v\n", addMinToIndent(indent+basicIdent), value)
}
...
func getGreaterIndent(indent string) string {
	return fmt.Sprintf("%v%v", indent, basicIdent)
}
...
func (ast AST) ASTString() string {
	...
	for _, function := range ast.functions {
		tree = fmt.Sprintf("%v%v", tree, function.ASTString(basicIdent))
	}

	tmpIndent = fmt.Sprintf("%v%v", basicIdent, basicIdent)

	tree = fmt.Sprintf("%v%v", tree, addIndAndNewLine(basicIdent, "int main()"))

	stmt := ast.main
	for stmt.GetNext() != nil {
		//basicIdent += "- "
		//tree = fmt.Sprintf("%v\n\n%v\n\n", tree, reflect.TypeOf(stmt))
